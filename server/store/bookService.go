...
func AddBook(db *sql.DB, userID, title, author string, numChapters int64, link *string, courseID *int64) (Item, error) {
	...
	_, err := db.Exec(`
		INSERT INTO books (user_id, title, author, numChapters, completedChapters, link, course_id)
		VALUES (?, ?, ?, ?, 0, ?, ?);
...
// UpdateCompletedChapters updates a book owned by user.
func UpdateCompletedChapters(db *sql.DB, userID string, id int64, completed int64) (Item, error) {
	res, err := db.Exec(`
		UPDATE books
